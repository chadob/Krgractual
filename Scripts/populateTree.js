var treePeople = {};
treeJSON = d3.json("/Data/familyTree.json", function(error, treeData) {
    ...
    // size of the diagram
    var viewerWidth = $(window).width() + 15;
...
console.log(links)
        ...
        .on("mouseover", function(d) {
          nodes = tree.nodes(d);
          domnode = this;
          d3.select(this).style("fill", "#cf6766");
          textHover(d, domnode, nodes);
        })
    ...
    function highlightChildren(source) {

    }

    function textHover(d, domnode, nodes) {
      draggingNode = d;
      if (nodes.length > 1) {
        links = tree.links(nodes);
        nodePaths = svgGroup.selectAll("path.link")
